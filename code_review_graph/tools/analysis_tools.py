...
def get_hub_nodes_func(
    ...
    Args:
        ...
        detail_level: "standard" returns full hub data; "minimal"
            returns only name, kind, and degree per hub.
    """
    ...
    if detail_level == "minimal":
        ...
        return {
            "status": "ok",
            "summary": f"Found {len(hubs)} hub node(s)",
            "hub_nodes": minimal_hubs,
            "count": len(hubs),
...
def get_bridge_nodes_func(
    ...
    Args:
        ...
        detail_level: "standard" returns full bridge data; "minimal"
            returns only name, kind, and centrality per bridge.
    """
    store, _root = _get_store(repo_root or None)
    ...
    if detail_level == "minimal":
        ...
        return {
            "status": "ok",
            "summary": f"Found {len(bridges)} bridge node(s)",
            "bridge_nodes": minimal_bridges,
            "count": len(bridges),
            "next_tool_suggestions": suggestions,
        }
    return {
...
def get_knowledge_gaps_func(
    ...
    Args:
        repo_root: Repository root (auto-detected if omitted).
        detail_level: "standard" returns full gap details; "minimal"
            returns only the count summary per category.
    """
    store, _root = _get_store(repo_root or None)
    ...
    if detail_level == "minimal":
        return {
            "status": "ok",
            "summary": f"{total} gap(s) across {len(gaps)} categories",
            "total_gaps": total,
            "summary_by_category": count_summary,
            "next_tool_suggestions": suggestions,
        }
    return {
        "status": "ok",
...
def get_surprising_connections_func(
    ...
    Args:
        ...
        detail_level: "standard" returns full edge data; "minimal"
            returns only source, target, and surprise_score per connection.
    """
    store, _root = _get_store(repo_root or None)
    ...
    if detail_level == "minimal":
        ...
        return {
            "status": "ok",
            "summary": f"Found {len(surprises)} surprising connection(s)",
            "surprising_connections": minimal,
            "count": len(surprises),
            "next_tool_suggestions": suggestions,
        }
    return {
...
def get_suggested_questions_func(
    ...
    Args:
        repo_root: Repository root (auto-detected if omitted).
        detail_level: "standard" returns full question data; "minimal"
            returns only question text for the top 5, grouped by priority count.
    """
    store, _root = _get_store(repo_root or None)
    ...
    if detail_level == "minimal":
        return {
            "status": "ok",
            "summary": f"{len(questions)} question(s) generated",
            "count": len(questions),
            "by_priority": {k: len(v) for k, v in by_priority.items()},
            "top_questions": [q.get("question", "") for q in questions[:5]],
            "next_tool_suggestions": suggestions,
        }
    return {
        "status": "ok",
